...
logger = logging.getLogger("internal.bmkg")
...
def extract_first_item(raw: str) -> Optional[dict]:
    """从 API 响应取第一条 Infogempa.gempa[0]。"""
    try:
        obj = json.loads(raw)
        infogempa = obj.get("Infogempa") or {}
...
def _loop() -> None:
    reg = get_source_status_registry()
    reg.register(SOURCE_ID, "BMKG 速报", "list")
    reg.set_connected(SOURCE_ID, True)
    while not _stop.is_set():
        try:
            _poll_once()
        except Exception as e:
            logger.exception("BMKG 轮询异常")
            reg.record_error(SOURCE_ID, str(e))
        _stop.wait(get_poll_interval("bmkg"))


def start() -> threading.Thread:
    _stop.clear()
    t = threading.Thread(target=_loop, name="Internal-BMKG", daemon=True)
    t.start()
    return t
